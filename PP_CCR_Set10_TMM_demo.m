...
flag = 5;       % flag = 0 - only GR, ANR, A+, CCR, and bicubic methods, the other get the bicubic result by default
                % flag = 1 - all the methods are applied
ressss = cell(1,3);
for upscaling = [2]; % the magnification factor
input_dir = 'test';
...
for d = 10   %1024
    ...
    fname = ['CCR_x' num2str(upscaling) '_K' num2str(dict_sizes(d)) '_max' num2str(nn_patch_max) '.mat'];
