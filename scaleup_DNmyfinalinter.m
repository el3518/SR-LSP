function [imgs, midres] = scaleup_DNmyfinalinter(conf, imgs,layer_use,n)
...
%
%imgsim{r} = im_h;
load([n '_inter_SRCDA_x' num2str(conf.upsample_factor) '.mat'])
featuresim = conf_use.V_pca'*collect(conf, imgsim, conf.upsample_factor, conf.filters); 

Dsim = pdist2(single(conf_use.dict_lores)', single(featuresim)');
