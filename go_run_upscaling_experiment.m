...
for upscaling = [2]; % the magnification factor x2, x3, x4...
image_number = 19; % 5 or 14
input_dir = 'testset'; %num2str(image_number)]; % Directory with input images from Set5 or Set14 image dataset
    ...
    for i = 1:numel(conf.filenames)
        ...
        for j = 1:numel(conf.desc)            
            ...
            if ~isempty(imgCB{1})
                ...
                rgbImg = ycbcr2rgb(rgbImg);rgbImg(:,:,1);255*img{1};
            else
