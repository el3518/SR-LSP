...
%input_dir = 'self/trainpepper';
%input_dir = 'downtestset_3';
...
tag = [input_dir '_x' num2str(upscaling) ];
    ...
    if exist([mat_file '.mat'],'file')
        ...
        selfconf=conf;
        save(mat_file, 'conf');save(selfmat_file, 'selfconf');
        % train call        
    end
...
% h [4 5 6 7 11 15 17 18 21 23 26 27 29 31]
 layer{3}=[4 5 6 7 11 15 17 18 21 23 26 27 29 31];%    x3
 layer{2}=[2 7 9 13 18 26 27 28 31];  %x2  coneximg{32};
  layer_use=layer{upscaling};
...
conhimg=getconhimg(himg,conf);
conlimg=getconlimg(upimg,conf);
%imshow(conlimg{37});
%im_b=img{1};
...
%exlimg{j}=conlimg{j}(:,:,i);
    ...
    coneximg{i}=exhimg;
    %conexlimg{i}=exlimg;coneximg{18};
end
      % load('pre_images.mat');
    ...
    [~, plores] = collectSamplesScales(conf, conlimg, 12, 0.98);  
    l2 = sum(plores.^2).^0.5+eps;
        l2n = repmat(l2,size(plores,1),1);    
        l2(l2<0.1) = 1;
        plores = plores./l2n;
        dic_size=1024;
        nn_patch_max=2048;lambda=0.00008;
         dic_plores = collect(conf, conlimg, conf.upsample_factor, {});
         
    %{    
    clu_num=1024;
    folder_current = pwd;
    run([folder_current, '\vlfeat-0.9.19\toolbox\vl_setup.m']);
    [centers, assignments] = vl_kmeans(plores, clu_num, 'Initialization', 'plusplus');
    num_centers_use=6;nn_patch_max=2048;lambda=0.1;
    assignments = double(assignments);
    %}
    for ii=1:length(layer_use)
        
        [~, phires] = collectSamplesScales(conf, coneximg{layer_use(ii)}, 12, 0.98); 
...
%}
 dic_phires = collect(conf, coneximg{layer_use(ii)}, conf.upsample_factor, {});
  ...
  % conf.result_dirGND = qmkdir(['ResGND-' sprintf('%s_x%d', input_dir, upscaling)]);
 %  conf.result_dirRGB = qmkdir(['pepperRGB-' sprintf('%s_x%d', input_dir, upscaling)]);
  % conf.result_dirGND = qmkdir(['pepperGND-' sprintf('%s_x%d', input_dir, upscaling)]);
    %%
    t = cputime;    
...
%}
        ...
        %low=img;
           ...
           res{1} = scaleup_DNksvdsim(conf, low,layer_use);
           % res{1} = scaleup_DNksvdsimself(conf,selfconf, low,layer_use,n);
            % res{1} = scaleup_DNmyfinal(conf,selfconf, low,layer_use,n);
            % res{1} = scaleup_DNmyfinalinter(conf, low,layer_use,n);
            toc(startt)
      ...
      %  for j = 1:numel(conf.desc)            
           ...
           % imwrite(result(:, :, j), conf.results{i}{j});
            ...
            conf.resultsRGB{i}{j} = fullfile(conf.result_dirRGB, [n sprintf('_%s_x%d', conf.desc{j}, upscaling) x]);
             %conf.resultsGND{i}{j} = fullfile(conf.result_dirGND, [n sprintf('_%s_x%d', conf.desc{j}, upscaling) x]);
      ...
      %  end      
       ...
       % imwrite(resultim, conf.resultsGND{i}{j});
