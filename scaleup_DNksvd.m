function [imgs, midres] = scaleup_DNksvd(conf, imgs,layer_use)
...
%simfeatures = collect(conf, conslimg, conf.upsample_factor, {}); 
 features = conf.V_pca'*collect(conf, conlimg, conf.upsample_factor, conf.filters); 
 % features = collect(conf, conlimg, conf.upsample_factor, {}); 
    end
conv2h_data = zeros(hei, wid, conv2_filters);
%num=20;
for ii=1:length(layer_use)%features=rand(30,100);
   % simhfeatures = collect(conf, conexsimg{ii}, conf.upsample_factor, {});
       Dc = pdist2(single(centers)', single(features)');
       
        % Dc = pdist2(single(rand(81,12))', single(rand(81,24))');
       [valc idxc] = min(Dc);
        ...
        for l = 1:size(features,2) 
            ...
            sumhfea =sumhfea + exp(-simval(ll))*simhfeatures(simidx(ll)) ;
             ...
             hfea(:,l)=sumhfea/dfea;
           %}
            D = pdist2(single(dicl{ii}{idxc(l)})', single(features(:,l))');
       [val idx] = min(D);
      hfea(:,l) = F{ii}{idxc(l)}{idx(1)} * features(:,l);
        end
...
%{
imgsim{r} = im_h;
featuresim = collect(conf, imgsim, conf.upsample_factor, {}); 
load([n '_SRCDA_x' num2str(conf.upsample_factor) '.mat'])
Dsim = pdist2(single(simcenters)', single(featuresim)');
